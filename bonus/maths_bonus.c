...
/*   Updated: 2023/03/02 13:47:44 by laprieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	define_iterations(t_data *data, t_complex z, t_complex c)
{
		...
		&& ((z.r * z.r) + (z.i * z.i)) <= 8.00)
	{
		...
		{
			z.r = fabsl((tmp.r * tmp.r) - (tmp.i * tmp.i) + c.r);
			z.i = fabsl(2.00 * (tmp.r * tmp.i) + c.i);
		}
		iterations++;
	}
	return (iterations);
...
t_complex	convert_coordinates(int x, int y, t_data *data)
{
	...
	res.r = ((((4.00 * d_x) / d_win_w) - 2.00) / data->zoom) + data->move_x;
	res.i = ((((4.00 * d_y) / d_win_h) - 2.00) / data->zoom) - data->move_y;
	return (res);
}
